...
### Imports ###
import sys, pickle, os, time, random                # 1) used to print text slowly, 2) save and load the game(funnyest thing i've ever read), 3) clear the screen also stops code for a breef time, 4) the clue is in the name, makes sudo random numbers                                   
# os.system("mode con cols=148 lines=45")             # window size for the "game" (used in the exe file. not nessesery to have otherwise)
...
class player:
    ...
    ### counters ###
        ...
        # encounters
        self.entered_a8 = False
        self.entered_d8 = False
        self.text_final_open = False
        # bosses defeated
        self.bosses_won = 0
    ...
    ### Puzzles ###
        self.solved_places = {'a3': False, 'a4': False, 'a5': False, 'a6': False, 'a7': False,
            ...
            'd1': False, 'd2': False, 'd3': False, 'd5': False, 'd6': False, 'd7': False,
...
map_locations = {
    ...
    'a5': {
        ...
        DESCRIPTION: 'An ancient forrest',
    ...
    'd8': {
        ZONENAME: "Garage?",
        DESCRIPTION: 'a lone garage in the mushroom plains',
        EXAMINATION: 'not sure if i expected that.',
        UP: "e8",
        DOWN: 'c8',
        LEFT: 'd7',
        RIGHT: '',
...
def Title_Screen():
    ...
    while True:
        ...
        print("#                                                                 - Help -                                                                         #")
        print("#                                                                 - Quit -                                                                         #\n")
        print("                                                     A game made by Dyadatola (not the charictors)\n")
...
def Setup_Game():
    ...
    while in_setup == True:
        ...
        print(" Note:\n @ You can only make it a maximum 6 charictors long. e.g: \"Dynomi\"\n @ It has to be at least 2 charictors long.          e.g: \"ME\"\n @ Canot choose special charictors.                  e.g: \"#\"\n @ Use only letters and numbers.                     e.g: \"A\" and \"1\"\n @ It has to start with a letter.                    e.g: \"A\" \n\n")
        player_name = input("> ")
        ...
        if player_name == '':
            ...
            Clear_Screen()
            print(" please select somthing valid without spaces, a leter has to exist as the first charictor before the numbers\n")
            print("(press enter to continue)")
            option = input("> ")
            in_setup = True
            continue
            ...
            in_setup = True
            continue
        elif 'Dyadatola' in player_name.lower():
            Clear_Screen()
            print(" nope\n")
            print("(press enter to continue)")
            option = input("> ")
        ...
        for i in player_name:
            if counter_i == 0:
                if i not in posible_options:
                    Clear_Screen()
                    print(" please select somthing valid without spaces, a leter has to exist before a number\n")
                    print("(press enter to continue)")
                    option = input("> ")
                    forbiden = True
                else:
...
def Setup2(): # your almost there
    ...
    while in_role_selection:
        ...
        print("     Note: it doesnt really matter what you choose")
        option = input("> ")
...
def Print_Location():
    ...
    print("                                                                 " + "#" * (4 + len(player1.location)))
    print("\n                                                  " + (" " * (16 - len(map_locations[player1.location][DESCRIPTION]))) + (map_locations[player1.location][DESCRIPTION]))
...
def Player_Prompt(): # welcome, to the darasic park
    ...
    print("(1-Move, 2-Inspect, 3-Status, 4-Map, 5-Save, 6-Check inventory, 7-Dance?, 8-Quit)\n")
    option = input("> ")
    ...
    if option.lower() in ['1', 'move', 'go', 'travel', 'walk']:      # go johhny go
            ...
            if player1.ability == 'NOTHING':
                ...
                if player1.location in ['a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'b2', 'b3', 'b4', 'b5', 'b6', 'b7', 'b8', 'c1', 'c2', 'c3', 'c4', 'c6', 'c7', 'c8', 'd1', 'd2', 'd3', 'd5', 'd6', 'd7', 'd8', 'e2', 'e3', 'e4', 'e5', 'e6', 'e7', 'e8']:
                    ...
                    if number == 1:
                        Funny_Encounter2()
                        player1.funny_counter += 1
            elif player1.enemy_times_killed == 0 and player1.funny_counter == 2:
        ...
        print("(press enter to continue)")
        option = input("> ")
    elif option.lower() in ['6', 'check', 'inventory', 'check inventory']:
        Inv()
    elif option.lower() in ['8', 'quit']:
        Clear_Screen()
        print("#" * 148)
        print('Are you sure?\n')
        print('(1-Yes, 2-no)')
        option = input("> ")
        possible_options = ['1', '2', 'yes', 'y', 'no', 'n']
        while option.lower() not in possible_options:
            print(" please select and option")
            option = input("> ")
        if option.lower() in ['1', 'yes', 'y']:
            global ingame
            ingame = False
        else:
            pass
...
def Print_Map():
    ...
    for i in zonenames:
        ...
        if counter / 8 == 1.0:
            ...
            print("                                                _________________________________________")
            to_print = "                                                "
    print("\n                                                Some Boss Locations      Other Locations")
    print("\n                                                a2 - Mr Groove           a1 - Home"
        + "\n                                                b3 - Maestro             a8 - Castel"
        + "\n                                                d4 - Q36u43e25e46n       b1 - Shop"
        + "\n                                                e1 - Big $1im3           d8 - Garage"
        + "\n                                                f7 - Bark Soul"
        + "\n                                                g7 - Lancer"
        + "\n                                                :) - ???\n")
    print("#" * 148)
...
def Player_Move(myAction):
    ...
    while in_location_selection:
        ...
        if option.lower() in ['a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8', 'b1', 'b2', 'b3', 'b4', 'b5', 'b6', 'b7', 'b8', 'c1', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7', 'c8', 'd1', 'd2' , 'd3', 'd4', 'd5', 'd6', 'd7', 'd8', 'e1', 'e2', 'e3', 'e4', 'e5', 'e6', 'e7', 'e8', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'g1', 'g2', 'g3', 'g4', 'g5', 'g6', 'g7', ':)']:
            ...
            in_location_selection = False
# examine the location and let the player choise the option to choose 'Fight'(boss or enemy)/'Puzzle'/'Back'
def Player_Examine():
    Clear_Screen()
    print("#" * 148)
    print("\n                                                               You are at:\n")
    Print_Location()
    print("\n                                   " + (" " * (45 - len(map_locations[player1.location][EXAMINATION]))) + (map_locations[player1.location][EXAMINATION]))
    print("#" * 148)
...
def Home():
    Clear_Screen()
    if player1.boss9_defeated == False or player1.boss8_defeated == False:
        ...
        if (player1.solved_places['a3'] and player1.solved_places['a3'] and player1.solved_places['a4'] and player1.solved_places['a5'] and player1.solved_places['a6'] and player1.solved_places['a7'] and\
            ...
            player1.solved_places['d1'] and player1.solved_places['d2'] and player1.solved_places['d3'] and player1.solved_places['d5'] and player1.solved_places['d6'] and player1.solved_places['d7'] and\
...
def Shop():
    ...
    while in_shop:
        ...
        print(f"                                                    1.) Potions: 30 Gold (You have {str(player1.pots)} Potions)")
...
def How_Many_Potions(): # potion seller, im going into battle and i need your strongest potions
    ...
    option = input("> ")
    if option.lower() in ['quit', 'exit', 'back', 'cancel', 'return']:
        Clear_Screen()
        print("#" * 148)
        print(" Shopkeeper:")
        print("    good bye\n") # see what i did there ;)
        ...
        for i in option:
            if i not in posible_options:
                Clear_Screen()
                print("#" * 148)
                print(" please select somthing valid without spaces\n")
                print("(press enter to continue)")
                option = input("> ")
                break
        try:
...
def Shop_talk(): # my potions are too strong for you traveler
    in_talk = True
    while in_talk == True:
        Clear_Screen()
        print("#" * 148)
        ...
        if player1.funny_counter >= 1:
            print("                                                    7.) Who is Dyadatola?")
            posible_options.append('7')
            posible_options.append('who is Dyadatola')
            posible_options.append('who is Dyadatola?')
        ...
        if option.lower() in ['1', 'who are you', 'who are you']:
            ...
            print("(press enter to continue)")
            option = input("> ")
        elif option.lower() in ['7', 'who is Dyadatola', 'who is Dyadatola?', 'who is king', 'who is king?', 'bosses', 'bosses?']:
            print(" Shopkeeper:")
            if option.lower() in ['7']:
                if player1.funny_counter >= 1:
                    print("    ah Dyadatola, the former king of this land. really cool dude. surprisingly no one knows him too well, even after so many years.")
                    print("    he seems nice at first. but then he starts to lose it, goes crazy.")
                    print("    Dyadatola says somthing along the lines of \"you said that last time\" and \"you are all the same\" to all of us")
            ...
            elif option.lower() in ['who is Dyadatola', 'who is Dyadatola?']:
                print("    ah Dyadatola, the former king of this land. really cool dude. surprisingly no one knows him too well, even after so many years.")
                print("    he seems nice at first. but then he starts to lose it, goes crazy.")
                print("    Dyadatola always says something along the lines of \"you said that last time\" and \"you are all the same\" to all of us")
                print("    id be cearfull around him, he knows more then you think\n")
...
def Quest(): # this is pointles honestly. i regret making ths part
    ...
    if player1.quest_counter == 7:
        ...
        if player1.quest_selected == True:
            ...
            while option.lower() not in possible_options:
                print(" please select something valid")
                option = input("> ")
            if option.lower() in ['1', 'y', 'yes', 'ya', 'ya dude', 'yas']:
                Clear_Screen()
                print("#" * 148)
...
### Garage (Gold) ###
# introductionto the gambling addict
def Funguy():
    Clear_Screen()
    funguys_name = " Funguy: "
    sprite = "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"\
    ...
    to_print = "    OH BEANS!!!\n"
    Print_Dialog(to_print)
    to_print = "    oh wait, you cant hurt me, haha\n\n"
    Print_Dialog(to_print)
    print("(press enter to continue)")
    option = input("> ")
    Clear_Screen()
    print(sprite)
    print(funguys_name)  
    to_print = "    dont worry i wont hurt you. wana gamble?\n\n"
    Print_Dialog(to_print)
    print("(press enter to continue)")
    option = input("> ")
    player1.entered_d8 = True
# The place to be if you want to feel broke
def Garage():
    in_garage = True
    funguys_name = " Funguy: "
    while in_garage:
        ...
        print("#" * 148)
        print(funguys_name)
        print("     This is your chance to be rich")
        print("                                            v             How would you like to gamble?             v\n\n")
        print("                                                   (You have " + str(player1.gold) + " Gold)\n")
        print("                                                     1.) rock leaf snail")
        print("                                                     2.) heads or tails")
        print("                                                     3.) Talk")
        print("                                                     4.) Exit")
        option = input("> ")
        possible_options = ['1', 'rock leaf snail', 'rock', 'r', '2', 'heads or tails', 'heads', 'h', '3', '4', 'talk', 't', 'exit', 'e', 'quit', 'q', 'back', 'b']
        while option.lower() not in possible_options:
            print(" plese enter a valid option")
        if option.lower() in ['1', 'rock leaf snail', 'rock', 'r']:
            Rock_leaf_Snail()
        elif option.lower() in ['2', 'heads or tails', 'heads', 'h']:
            Heads_Tails()
        elif option.lower() in ['3', 'talk', 't']:
            Garage_talk()
            pass
        else:
            Clear_Screen()
            print(funguys_name)
            print("     Good Gamble\n") # this isn't funny the secont time
            print("(press enter to continue)")
            option = input("> ")
            in_garage = False
# how much to spend text (wanted to do it so that you select the price here, but i couldn't)
def Gamble_How_much():
    print("#" * 148)
    print(" Funguy: ")
    print("    lesgo\n")
    print(" How much GOLD are you willing to spend?")
# the rock paper sizor knock off
def Rock_leaf_Snail():
    Clear_Screen()
    Gamble_How_much()
    selecting = True
    ...
    while selecting:
        ...
        try:
            option = int(option)
            if option <= player1.gold and option != 0:
                win = option
                selecting = False
            elif option == 0:
                playing = False
            ...
            if option.lower() in ['exit', 'e', 'back', 'b', 'quit', 'q']:
                playing = False
                selecting = False
            else:
                print(" please select a valid number without spaces\n")
    if playing == True:
        # pick rock leaf or snail
        Clear_Screen()
        print("#" * 148)
        print(" Funguy:\n    ok go a head choose:")
        print("    (1-Rock, 2-Leaf, 3-Snail)\n")
        option = input("> ")
        possible_options = ['1', 'r', 'rock', '2', 'l', 'leaf', '3', 's', 'snail']
        while option.lower() not in possible_options:
            print(" plese enter a valid option")
            option = input("> ")
        if option.lower() in ['1', 'r', 'rock']:
            choice = 1
        elif option.lower() in ['2', 'l', 'leaf']:
            choice = 2
        elif option.lower() in ['3', 's', 'snail']:
            choice = 3
        funguys_choice = random.randint(1, 3)
        # rock vs rock
        if choice == 1 and funguys_choice == 1:
            to_print = ("    oh wow a draw,\n\n No one lost anything\n\n")
        # rock vs paper
        elif choice == 1 and funguys_choice == 2:
            to_print = (f"    oh, i won.\n\n You lost {win} GOLD\n\n")
            player1.gold -= win/3
        # rock vs scisor
        elif choice == 1 and funguys_choice == 3:
            to_print = (f"    aw, i lost.\n\n You won {win*3} GOLD\n\n")
            player1.gold += win*3
        # paper vs rock
        elif choice == 2 and funguys_choice == 1:
            to_print = (f"    aw, i lost.\n\n You won {win*3} GOLD\n\n")
            player1.gold += win*3
        # paper vs paper
        elif choice == 2 and funguys_choice == 2:
            to_print = ("    oh wow a draw,\n\n No one lost anything\n\n")
        # paper vs scisor
        elif choice == 2 and funguys_choice == 3:
            to_print = (f"    oh, i won.\n\n You lost {win} GOLD\n\n")
            player1.gold -= win/3
        # scisor vs rock
        elif choice == 3 and funguys_choice == 1:
            to_print = (f"    oh, i won.\n\n You lost {win} GOLD\n\n")
            player1.gold -= win/3
        # scisor vs paper
        elif choice == 3 and funguys_choice == 2:
            to_print = (f"    aw, i lost.\n\n You won {win*3} GOLD\n\n")
            player1.gold += win*3
        # scisor vs sisor
        elif choice == 3 and funguys_choice == 3:
            to_print = ("    oh wow a draw,\n\n No one lost anything\n\n")
        print(" Funguy: ")
        if funguys_choice == 1:
            print("    i choose rock")
        elif funguys_choice == 2:
            print("    i choose leaf")
        else:
            print("    i choose snail")
        print(to_print)
        print("(press enter to continue)")
        option = input("> ")
...
def Heads_Tails():
    ...
    if playing == True:
        ...
        print("#" * 148)
        print(" Funguy:\n    ok go on, choose:")
        print("    1-Heads or 2-Tails\n")
        option = input("> ")
        possible_options = ['1', 'h', 'heads', '2', 't', 'tails']
        while option.lower() not in possible_options:
            print(" plese enter a valid option")
            option = input("> ")
        if option.lower() in ['1', 'h', 'heads']:
            choice = 1
        elif option.lower() in ['2', 't', 'tails']:
            choice = 2
        funguys_choice = random.randint(1, 2)
        if funguys_choice == choice:
            to_print = (f"    aw, i lost.\n\n You won {win*2} GOLD\n\n")
            player1.gold += win*2
        else:
            to_print = (f"    oh, i won.\n\n You lost {win} GOLD\n\n")
...
def Garage_talk():
    Clear_Screen()
    # ask about the games, about him, what its like to live here, and why we cant leave
    in_talk = True
    while in_talk == True:
        Clear_Screen()
        print("#" * 148)
        print(" Funguy:")
        print("    Oh a question, what is it?")
        print("                                            v             What would you like to talk about?             v\n\n")
        print("                                                    1.) Who/What are you?")
        print("                                                    2.) Gambling?")
        print("                                                    3.) How to play: Rock leaf Snail?")
        print("                                                    4.) How to play: Heads or Tails?")
        print("                                                    5.) How do you escape this place?")
        print("                                                    6.) Back")
        posible_options = ['1', '2', '3', '4', '5', '6', 'who/what are you', 'who are you', 'what are you', 'gambling', 'how to play: rock leaf snail', 'how to play: heads or tails', 'how do you escape this place', 'back', 'b']
        option = input("> ")
        while option.lower() not in posible_options:
            print(" please select and option")
            option = input("> ")
        Clear_Screen()
        print("#" * 148)
        if option.lower() in ['1', 'who are you', 'what are you', 'who/what are you']:
            print(" Funguy:")
            print("    i dont know, you tell me\n")
            print("(press enter to continue)")
            option = input("> ")
        elif option.lower() in ['2', 'gambling']:
            print(" Funguy:")
            print("    yea, spend money for a chance to win.")
            print("    gamble responcibly\n")
            print("(press enter to continue)")
            option = input("> ")
        elif option.lower() in ['3', 'how to play: rock leaf snail']:
            print(" Funguy:")
            print("    you and i choose either rock leaf or snail")
            print("    rock beats scisor that beat paper which beats rock")
            print("    if your choice beats mine, you win.")
            print("    its actualy kinda easy, earn 3 times the amount you gave me\n")
            print("(press enter to continue)")
            option = input("> ")
        elif option.lower() in ['4', 'how to play: heads or tails']:
            print(" Funguy:")
            print("    you choose either heads or tails")
            print("    if i get the same thing that you choose you won")
            print("    very simple, earn 2 times the amount you gave me\n")
            print("(press enter to continue)")
            option = input("> ")
        elif option.lower() in ['5', 'how do you escape this place']:
            print(" Funguy:")
            print("    Hmm, they say you have to beat at least 6 bosses to exit")
            print("    id be careful if i were you.\n")
            print("(press enter to continue)")
            option = input("> ")
        elif option.lower() in ['6', 'back', 'b']:
            in_talk = False
...
def Funny_Encounter(): # ??? what just happened
    ...
    print(sprite)
    print(his_name)
    to_print = "    My name is Dyadatola. Whats yours?\n"
    Print_Dialog(to_print)
    ...
    option = input("> ")
    Clear_Screen()
    if "not a human" in option.lower() or "i am not a human" in option.lower():
        print(sprite3)
        print(his_name)
...
def Funny_Encounter2(): # ok i guess its kinda funny
    ...
    print("(press enter to continue)")
    option = input("> ")
# Funny encounter 3 (only avalible if you act and spare everyone)
def Funny_Encounter3():
    ...
    if option.lower() in ["2", "no", "nope", "nah"]:
        ...
        Print_Dialog(to_print)
        print("Frogerion: ")
        to_print = "    *ribit* waz popin Dyadatola. looking spiffy.\n"
        Print_Dialog(to_print)
        print("(press enter to continue)")
        option = input("> ")
        Clear_Screen()
        ...
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    		...
    		+ "\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@  @@@@@@@  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n")
        print("Muffet:")
        to_print = "    master Dyadatola. the pie is ready\n"
        Print_Dialog(to_print)
        print(his_name)
...
def Enemy_Introduction(): # allow me to introduce myself
    ...
    print("                                         " + (" " * (40 - len(to_print))) + f"{enemy.name} {to_print}" + "\n")
# the battle screen
def Fight(): # its show time
    ...
    while infight == True:
        print("#" * 148)
        print(f"                                                {player1.name}'s HP: {player1.hp}/{player1.maxhp}" + (" " * (8 - len(str(player1.maxhp)))) + f"{enemy.name}'s HP: {enemy.hp}/{enemy.maxhp}")
        print(f"                                                {player1.name}'s LEVEL: {player1.level}" + (" " * (9 - len(str(player1.level)))) + f"{enemy.name}'s GOLD: {enemy.gold} EXP: {enemy.exp}\n\n")
...
def Player_Attack():
    ...
    print("(press enter to continue)")
    option = input("> ")
    if enemy.name in ['Dyadatola', 'Bark Soul', 'eremthgin']:
        Check_Enemy_Mode()
    elif enemy.hp <= 0:
        Battle_Won()
        Level_Up()
    ...
    if enemy.name == 'eremthgin':
        ...
        if player1.job == 'NOTHING':
            ...
            if enemy.hp <= 9999999986890:
                ...
                skip_attack = True
        if enemy.hp <= 0:
            if player1.enemy_times_killed >= 100 or player1.job == 'NOTHING':
                pass
            else:
                E_Dialog_Last()
            Victory()
...
def Battle_Won():
    ...
    if enemy.hp <= 0: # how could you?
        if player1.enemy_times_killed == 0:
            print(":)")
            time.sleep(5.0)
            Clear_Screen()
        else:
...
def Check_Enemy():
    ...
    if enemy.name in ['Mr Groove', 'Maestro', 'eremthgin', 'Q36u43e25e46n', 'Bark Soul', 'Big $1im3', 'Muffet', 'Mettaton', 'Lancer']:
        ...
        # boss7 is skipped :)
            ...
            player1.killed_lancer == True
    if player1.text_final_open == False:
        if player1.bosses_won == 6:
            Clear_Screen()
            print("#" * 148)
            print(" The door has opened, he is waiting at :)")
            player1.text_final_open = True
            print("(press enter to continue)")
            option = input("> ")
            Clear_Screen()
    # geting wepons
        ...
        if player1.quest_selected == True:
            if quest.name =='The Begining':
                ...
                if enemy.name == 'Iseecle':
                    player1.quest1_count += 1
                    if player1.quest1_count == 2:
                        Clear_Screen()
                        player1.gold += quest.item
                        print("#" * 148)
                        print(" You compleated the quest " +  "\"" + quest.name + "\"")
                ...
                if enemy.name == 'Funguy':
                    player1.quest2_count += 1
                    if player1.quest2_count == 2:
                        Clear_Screen()
                        player1.gold += quest.item
                        print("#" * 148)
                        print(" You compleated the quest " +  "\"" + quest.name + "\"")
                ...
                if enemy.name == 'Zombie':
                    player1.quest3_count += 1
                    if player1.quest3_count == 8:
                        Clear_Screen()
                        player1.gold += quest.item
                        print("#" * 148)
                        print(" You compleated the quest " +  "\"" + quest.name + "\"")
                ...
                if enemy.name == 'Frogerion':
                    player1.quest4_count += 1
                    if player1.quest4_count == 4:
                        Clear_Screen()
                        player1.gold += quest.item
                        print("#" * 148)
                        print(" You compleated the quest " +  "\"" + quest.name + "\"")
...
def Before_The_Battle(): # ...
    ...
    Print_Dialog(to_print)
    time.sleep(5.0)
    to_print = "    friends, famliy. But and most importantly his best friend Dyadatola\n\n"
    print(" Zombie:    ")
    Print_Dialog(to_print)
    time.sleep(5.0)
    ...
    Clear_Screen()
    print("#" * 148)
    to_print = "    Dyadatola became very ill...\n\n"
    print(" Big guy:    ")
    Print_Dialog(to_print)
    time.sleep(5.0)
    Clear_Screen()
    ...
    Print_Dialog(to_print)
    time.sleep(5.0)
    to_print = "    right now Dyadatola is here, in this exact game, he is our king, he is waiting.\n\n"
    Print_Dialog(to_print)
    time.sleep(3.0)
    Clear_Screen()
    print("#" * 148)
...
def E_Dialog(): # i should realy come up with better names for these
    ...
    if player1.enemy_times_killed == 0:
        to_print = "    ... huh\n\n"
        Print_Dialog(to_print)
        to_print = "    you choose to defeat only the enemys with acting\n\n"
        print(his_name)
        Print_Dialog(to_print)
        ...
        print(his_name)
        Print_Dialog(to_print)
    elif player1.enemy_times_killed < 100:
...
def E_Dialog_Battle2():
    ...
    sprite = "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"\
           ...
           + "\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
    print(sprite)
    to_print = "    i dont actualy want to leave this place anymore\n\n"
    print(his_name)
    Print_Dialog(to_print)
    print("(press enter to continue)")
    option = input("> ")
...
def E_Dialog_Last():
    ...
    if option.lower() in posible_options:
        Clear_Screen()
        print("You huged Dyadatola, he fellt a little better shortly before passing away")
        time.sleep(5.0)
        print("(press enter to continue)")
        option = input("> ")
    Clear_Screen()
...
def Credits(): # you won, congrats
    ...
    print("                 ████████╗██╗░░██╗███████╗  ██████╗░██╗░░░░░██╗███╗░░██╗██████╗░  ░██████╗░██╗░░░██╗███████╗░██████╗████████╗\n"\
        ...
        + "                 ░░░╚═╝░░░╚═╝░░╚═╝╚══════╝  ╚═════╝░╚══════╝╚═╝╚═╝░░╚══╝╚═════╝░  ░░░╚═╝░░░░╚═════╝░╚══════╝╚═════╝░░░░╚═╝░░░")
    print("#" * 148)
    print("                                                          by Dyadatola (not the charictors)")
...
def Game_Over_Screen(): # you lost boo hoo, how sad :(
    ...
    if option.lower() in ['1', 'yes', 'sure', 'continue', 'ya dude', 'yas', 'ok']:
        ...
        if player1.gold < 20:
            ...
            player1.gold -= 20
    elif option.lower() in ['2', 'no', 'nah', 'nope', 'i wish to die', 'f']: # press f to pay respect
        responce = random.randint(1, 3)
        if responce == 1:
            to_print = " Good bye %s :) its been fun\n" % (player1.name)
        elif responce == 2:
            to_print = " Good bye %s :) hope you return soon\n" % (player1.name)
        else:
            to_print = " Good bye %s :) hope you kyboard/controler is ok\n" % (player1.name)
        Print_Dialog(to_print)
        print("(press enter to continue)")
        option = input("> ")
        Clear_Screen()
